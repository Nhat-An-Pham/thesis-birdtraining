  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://13.214.85.41/api/trainingcourse-customer/customer-bird?customerId=${customerId}`);
        ...
        <TableContainer>
            ...
            <Table>
                ...
                <TableRow key={bird.id}>
                    ...
                    <TableCell>{bird.description}</TableCell>
                    <TableCell>
                      <a href={bird.picture} target="_blank" rel="noopener noreferrer">
                        <img src={bird.picture} alt="Description of the image" style={{ width: '200px', height: '150px' }}/>
                      </a>
                    </TableCell>
                    <TableCell><button onClick={() => handleButtonBirdSkillReceivedClick(bird.id)}>View received bird skills</button></TableCell>
                </TableRow>
                ))}
                <Table>
                    {selectedBird != null && (
