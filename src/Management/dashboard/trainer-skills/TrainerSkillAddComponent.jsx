...
export default function TrainerSkillAddComponent({ open, handleClose }) {
  ...
  async function addTrainerSkill() {
    try {
      ...
      let res = await dashboardService.AddTrainerSkill(model);
      if (res.status === 200) {
        handleClose();
      } else {
  ...
  return (
    <>
      <Dialog open={open} onClose={handleClose} fullWidth>
        <DialogTitle>Add new trainer skill</DialogTitle>
        <DialogContent>
          <Stack spacing={3}>
